...
export function CompareClient({ initialSlug1, initialSlug2 }: CompareClientProps = {}) {
  // Safe fallbacks to prevent indexing collisions
  const defaultCol1 = collegesData.find(c => c.id === initialSlug1) ? initialSlug1! : collegesData[0].id;
  const defaultCol2 = collegesData.find(c => c.id === initialSlug2) ? initialSlug2! : collegesData[1].id;

  const [col1Id, setCol1Id] = useState(defaultCol1);
  const [col2Id, setCol2Id] = useState(defaultCol2);
